...
export function exportToTissXML<T extends Record<string, any>>(data: T[], filename: string): void {
  try {
    logger.info(`Exporting data to TISS XML: ${filename}`, { recordCount: data.length });
    ...
    logger.info(`TISS XML export completed: ${filename}`);
  } catch (error) {
    logger.error('Erro ao exportar dados para XML:', { error });
...
export function exportToFHIR<T extends Record<string, any>>(
  ...
  try {
    logger.info(`Exporting data to FHIR JSON: ${filename}`, { recordCount: data.length, resourceType });
    ...
    logger.info(`FHIR export completed: ${filename}`);
  } catch (error) {
    logger.error('Erro ao exportar dados para formato FHIR:', { error });
